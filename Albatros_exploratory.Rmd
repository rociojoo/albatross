...
#Icanot access the sensor table uploaded in github
sensor_table <-read.csv(text=GET("https://github.com/lmml2-sci/albatross/find/master/Logger_deployment_tbl.csv"), stringsAsFactors=F)
...
acc <- read.csv('Logger-Combined_data/acc_clean.csv', stringsAsFactors=F)
...
magnet <- read.csv('Logger-Combined_data/magnet_clean.csv', stringsAsFactors=F)
...
summ_visualizations <- function (id, duskam, dawnam){
  
  gps_id1<-gps[gps[,"id"]==id,]
  acc_id1<-acc[acc[,"id"]==id,]
  mag_id1<-magnet[magnet[,"id"]==id,]
...
x=1 # x equals the id
results <- summ_visualizations(id=params[x,1],duskam=params[x,2], dawnam=params[x,3])
```
...
m0 <- highlight_key(results$gps_id1)
